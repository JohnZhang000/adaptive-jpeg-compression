...
class proactiver_and_reactiver(nn.Module):
    def __init__(self,spectrum_ckpt, classifier_type, dataset):
    ...
    def forward(self,x, method):
        ...
        with torch.no_grad():
            ...
            # 标签汇总
            idx_adv     = np.in1d(label_spectrum_before.detach().cpu().numpy(),cls_adv)
            idx_rpl     = idx_adv
            if 2 != method:
                idx_rpl=[]
...
if __name__=='__main__':    
    ...
    if len(sys.argv)!=5:
        print('Manual Mode !!!')
        model_spectrum_dir = '../saved_tests/img_attack/spectrum_label/allconv/checkpoint.pth.tar'
    ...
    加载模型
    '''
    model = proactiver_and_reactiver(model_spectrum_dir, model_vanilla_type, 'cifar-10')
    ...
    accuracys = np.zeros((len(attacks),len(eps_L2)))
    fprint_list=[]
    file_log=os.path.join(saved_dir,'result_log.txt')
    f=open(file_log,'w')
    f.write('my defense result\n')
    for attack_idx,attack in enumerate(attacks):
        ...
        for eps_idx,eps in enumerate(eps_now):
            ...
            for i in range(batch_num):#tqdm(range(batch_num)):
                ...
                label_pred = model(attacked_img,method)
            ...
            accuracy_all = correct_all/len(labels_now)
            accuracy_pred_all = correct_pred_all/len(labels_now)
            accuracy_adv_all = correct_adv_all/len(labels_now)
            accuracys[attack_idx,eps_idx]=accuracy_all*100
            prt_info='[ATTACK]:%s  [Eps]:%.3f  Acc:%.1f  Acc_adv:%.1f  Acc_pred:%.1f'%(attack,eps,100*accuracy_all,100*accuracy_adv_all,100*accuracy_pred_all)
            print(prt_info)
            f=open(file_log,'a')
            f.write(prt_info+'\n')
            f.close()
