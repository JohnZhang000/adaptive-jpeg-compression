...
if __name__=='__main__':    
    ...
    if 'cifar-10'==dataset:
        ...
        images=images[0:int(images.shape[0]/10),...]
        labels=labels[0:int(labels.shape[0]/10),...]
        print('%d images',images.shape[0])
...
#    defences_names_aft.append('Roud')
    ...
    file_log=os.path.join(saved_dir,'result_pre_post_log_cd.txt')
    f=open(file_log,'w')
    ...
    for j in range(len(attacks)):
        ...
        images_adv_list=[]
        for i_attack in tqdm(range(batch_num)):
