...
class Net(nn.Module):
    def __init__(self,input_size):
        ...
        self.conv3 = nn.Conv1d(10, 20, 3, 1)
        
        self.linear1 = nn.Linear(20*2,20)
...
if __name__=='__main__':   
    ...
    if 'cifar-10'==dataset:
        model     = Net(g.spectrum_num_cifar).cuda()
    elif 'imagenet'==dataset:
        model     = Net(g.spectrum_num_imagenet).cuda()
    model.init_weights()
    ...
    end_time=time.time()
    prt_info=("[CNN] Train Time %f s") % (end_time-start_time)
    print(prt_info)
    logging.info(prt_info)
    prt_info=("[CNN] Train acc %f") % best_acc
    print(prt_info)
    logging.info(prt_info)
