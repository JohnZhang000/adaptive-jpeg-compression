...
class proactiver_and_reactiver(nn.Module):
    def __init__(self,spectrum_ckpt, adaboost_pkl, svm_pkl, classifier_type, dataset):
        ...
        if 'imagenet'==dataset:
            ...
            self.spectrum_size=int(self.img_size/2)
            self.spectrum_mean_std=[0,1.0]
            self.adv_cls_label=g.nb_classes_imagenet+1
            self.fft_transformer1 = img_transformer(g.levels_all_imagenet,g.levels_start_imagenet,g.levels_end_imagenet)
            self.fft_transformer2 = img_transformer(g.levels_all_imagenet,g.levels_start_imagenet,g.levels_end_imagenet+1)
            
            self.spectrum_net = spectrum_net_imagenet(self.spectrum_size)
            ...
            self.fft_transformer1 = img_transformer(g.levels_all_cifar,g.levels_start_cifar,g.levels_end_cifar)
            self.fft_transformer2 = img_transformer(g.levels_all_cifar,g.levels_start_cifar,g.levels_end_cifar)
            self.adv_cls_label=g.nb_classes_imagenet+1
            
            self.spectrum_net = spectrum_net_cifar(self.spectrum_size)
            checkpoint = torch.load(spectrum_ckpt)
            self.spectrum_net.load_state_dict(checkpoint['state_dict'])
            self.spectrum_net.cuda().eval()
        else:
...
if __name__=='__main__':    
    ...
    if 'cifar-10'==dataset:
        ...
        data_dir=os.apth.join(g.dir_imagenet,'val')
    ...
    for attack_idx,attack in enumerate(attacks):
        ...
        for eps_idx,eps in enumerate(eps_now):
            if ((('CW_L2_IDP'==attack) or ('Deepfool_L2_IDP'==attack_idx)) and (0 != eps_idx)):
                continue
                                 
            attacked_img = get_attacked_img(model.fmodel,images_now,labels_now,batch,attack,eps,dataset)
            
            for sc_idx,spectrum_classifier in enumerate(spectrum_classifiers):
                for pre_idx,preprocesser in enumerate(preprocessers):
                    correct,correct_adv,correct_pred=my_pred(attacked_img,labels_now,batch,method,spectrum_classifier,preprocesser)
                
                    prt_info='[ATTACK]:%s  [Eps]:%.3f  [%s] [%s] Acc:%.1f  Acc_pred:%.1f  Acc_adv:%.1f'%(attack,eps,spectrum_classifier,preprocesser, 100*correct,100*correct_pred,100*correct_adv)
                    print(prt_info)
    ...
    np.save(os.path.join(saved_dir,'result_log.npy'),results)