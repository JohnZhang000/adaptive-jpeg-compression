...
def loss_grad_bpda(model,x_orig,x_adv_def,x_adv_bpda,y_adv):
    ...
    def save_grad():
        def hook(grad):
            ...
            grad.data.zero_()
        return hook
    handle=x_adv_bpda.register_hook(save_grad())

    diff=x_adv_bpda.cuda()-x_orig.cuda()
...
def BPDA(model,x_orig,y_orig,mean,std,classes=10,epoch=3,defend=None):
    ...
    for i in range(epoch):
        ...
        p,g,L2=loss_grad_bpda(model,x_orig_tc,x_adv_def_tc,x_adv_bpda_tc,y_adv)
        x_adv_bpda_tc -= LR * g
...
if __name__=='__main__':    
    ...
    dataset=g.load_dataset(dataset_name,data_setting.dataset_dir,'val',0.02)
    ...
    for i,(x_orig,y_orig) in enumerate(tqdm(dataloader)):
        ...
        ret=BPDA(model,x_orig,y_orig,mean,std,classes=classes,epoch=50,defend=defend_webpf_wrap)
